import 'dart:io';
import 'dart:convert';
import 'package:forum_flutter/model/support/ErrorListener.dart';
import 'package:http/http.dart';

enum TypeHeader { json, urlencoded }

class RestManager {
  ErrorListener? delegate = BasicErrorListener();
  String? token;
  ...
  Future<String> _makeRequest(
    ...
    while (true) {
      try {
        ...
        if (delegate != null && !errorOccurred) {
          delegate!.errorNetworkOccurred('$err\n$st');
          errorOccurred = true;
        }
  ...
  Future<String> makeGetRequest(String serverAddress, String servicePath,
      [Map<String, String>? value, TypeHeader type = TypeHeader.json]) async {
